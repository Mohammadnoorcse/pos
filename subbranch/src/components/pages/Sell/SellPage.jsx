...
export default function SellPage() {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // Which account the money actually landed in — backend expects
      // 'cash' or a banks.code (e.g. the MFS/bank identifier the payment
      // modal captured). Falls back to 'cash' if nothing was picked.
      const account = cashPay
        ? "cash"
        : cardPay
          ? cardPay.account || cardPay.method || "cash"
          : multiPay
            ? multiPay.bank > 0
              ? multiPay.bankAccount || "cash"
              : "cash"
            : "cash";
      ...
      // Clear cart/discount/payment state now that the sale is saved —
      // keeps confirmedSale/invoiceNo intact so the confirmation modal &
      // print memo still show correct data, but prevents an accidental
      // double-submit (duplicate sale + double stock deduction) if the
      // cashier clicks "Confirm Order" again.
      setCart([]);
      setSelectedCustomer(null);
      setDiscount(null);
      setOthers(null);
    ...
    >
      ...
      <div className="flex-1 flex gap-4 p-4 min-h-0">
        ...
        <div className="w-[280px] shrink-0 flex flex-col gap-3">
          ...
          {saleError && (
            <div
              className="rounded-lg px-3 py-2 text-[11.5px] font-semibold text-center"
              style={{ backgroundColor: C.vermillionTint, color: C.vermillion }}
            >
              {saleError}
            </div>
