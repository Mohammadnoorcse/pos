...
export function AllDamageProduct() {
  ...
  const loadRecords = async () => {
    try {
      ...
      // Sub-branch ইউজার শুধু নিজের ব্রাঞ্চের ড্যামেজ প্রোডাক্টই দেখবে —
      // অন্য কোনো ব্রাঞ্চের ডেটা দেখানো যাবে না।
      if (SUB_BRANCH_ID) params.branch_id = SUB_BRANCH_ID;
      const data = await fetchDamageRecords(params);
      let rows = data.data || data || [];
      if (SUB_BRANCH_ID) {
        rows = rows.filter((r) => {
          const branchId = r.branch_id ?? r.branch?.id ?? null;
          // যদি রেকর্ডে branch id না থাকে, তাহলে সন্দেহের অবকাশ না রেখে বাদ দেওয়া হচ্ছে।
