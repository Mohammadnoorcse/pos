...
export function CrmPermissionsTab({ canManage = false }) {
  ...
  const loadPermissions = async () => {
    try {
      ...
      // Backend controller paginate response দেয় (data Array)
      setPermissions(response.data || []);
    } catch (err) {
      console.error("Error loading CRM permissions:", err);
      setError("CRM পারমিশন লিস্ট লোড করতে সমস্যা হয়েছে।");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (name) => {
    try {
      ...
      alert(err.message || "পারমিশন তৈরি করা সম্ভব হয়নি। ইউনিক নাম ব্যবহার করুন।");
    }
  };
  ...
  const handleUpdate = async (name) => {
    ...
    try {
      ...
      alert(err.message || "পারমিশন আপডেট করতে সমস্যা হয়েছে।");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert("পারমিশন ডিলিট করতে সমস্যা হয়েছে।");
    }
  };
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-left" style={{ color: COLORS.muted }}>
                ...
                <th
                  className="font-semibold text-[11px] uppercase tracking-wide pb-2.5 px-2.5 border-b"
                  style={{ borderColor: COLORS.line, display: canManage ? "table-cell" : "none" }}
                >
                  Actions
                </th>
              </tr>
            ...
            <tbody>
              {permissions.map((perm, i) => (
                ...
                >
                  ...
                  <td className="py-3 px-2.5" style={{ display: canManage ? "table-cell" : "none" }}>
