  ...
  const loadLedgerData = async () => {
    ...
    try {
      const [purchasesRes, paymentsRes, returnsRes] = await Promise.all([
        fetchPurchases({ supplier_id: supplierId, per_page: 500, ...(SUB_BRANCH_ID ? { branch_id: SUB_BRANCH_ID } : {}) }),
        fetchSupplierPayments({ supplier_id: supplierId, per_page: 500, ...(SUB_BRANCH_ID ? { branch_id: SUB_BRANCH_ID } : {}) }),
        fetchPurchaseReturns({ supplier_id: supplierId, per_page: 500, ...(SUB_BRANCH_ID ? { branch_id: SUB_BRANCH_ID } : {}) }),
      ]);
