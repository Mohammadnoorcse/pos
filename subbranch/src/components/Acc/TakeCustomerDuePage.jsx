...
import {
  ...
  fetchDueCollections,
  createDueCollection,
} from "../../api/acc/customerDueService"; 
import {
  Users,
  Search,
  ChevronDown,
...
function initials(name) {
  return (name || "C").split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase();
}
...
/* ---------------------------------------------------------------------
   Helper: safely pull an array out of an API response no matter how
   it's wrapped (raw array, {data: [...]}, {customers: [...]},
   {data: {customers: [...]}}, etc).
--------------------------------------------------------------------- */
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0" style={{ borderColor: COLORS.line }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: COLORS.muted }}>ID: {customer.id} · {customer.area || "N/A"}</p>
            </div>
          </div>
            ...
            <Field label="Note">
              <div className="flex items-start gap-2 rounded-lg border px-3 py-2" style={{ borderColor: COLORS.line }}>
                ...
                <textarea
                  value={form.note}
                  onChange={set("note")}
                  placeholder="e.g. Partial payment against due invoice"
                  rows={2}
...
export function TakeCustomerDuePage({ onNavigate }) {
  ...
  const loadData = async () => {
    try {
      ...
      // Handles any of these response shapes:
      //   [ ... ]
      //   { customers: [...] }              <- current API shape
      //   { data: [...] }
      //   { data: { customers: [...] } }
      const rawCustomers = extractArray(custRes, "customers");
      const rawCollections = extractArray(collRes, "collections");

      // 1. Map collections
      const mappedCollections = rawCollections.map((c) => ({
        ...c,
        amount: Number(c.amount || 0),
        date: c.collected_date
          ? c.collected_date.split("T")[0]
          : c.created_at
          ? c.created_at.split("T")[0]
          : c.date || "",
        customerId: c.customer_id || c.customer?.id,
      }));
      ...
      const mappedCustomers = rawCustomers.map((c) => {
        return {
          ...
          area: c.area || c.address || "N/A",
          totalDue: Number(c.total_due ?? c.due ?? c.totalDue ?? 0),
          agingDays: Number(c.aging_days ?? c.aging_days_count ?? c.agingDays ?? 0),
          lastInvoice: c.last_invoice ?? c.last_invoice_no ?? c.lastInvoice ?? "N/A",
        };
      });

      setCustomers(mappedCustomers);
    } catch (err) {
      console.error("Error loading customer due data:", err);
      setError("কাস্টমার বাকির ডাটা লোড করতে সমস্যা হয়েছে।");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = customers
    ...
    .filter((c) => {
      const matchesQuery = [c.name, c.phone, c.area, c.id]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
  ...
  const handleSave = async (customerId, form) => {
    const cust = customerOf(customers, customerId);
    try {
      await createDueCollection({
        customer_id: customerId,
        amount: form.amount,
        account: form.account,
        method: form.account === "cash" ? "Cash" : "Bank Transfer",
        note: form.note,
        collected_date: form.date,
      });
      setSelected(null);
      notify(`${taka(form.amount)} collected from ${cust?.name || customerId}`);
      loadData();
    } catch (err) {
      console.error("Error recording collection:", err);
      alert("কালেকশন রেকর্ড করা সম্ভব হয়নি।");
    }
  };
