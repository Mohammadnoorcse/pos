...
import { fetchCashFlowEntries, createCashFlowEntry, deleteCashFlowEntry } from "../../api/acc/cashFlowService";
import {
  Wallet,
  Loader2,
  Search,
...
// Fixed opening balance for the ledger. If your API starts returning this
// value too (e.g. inside `summary`), swap this constant for that field.
...
// The rest of the UI displays dates as "DD-MM-YYYY".
...
// API entries have a numeric `id` (DB primary key) AND a human-facing
// `entry_no` (e.g. "CF-2201"). Anywhere we display a "code" to the user,
// prefer entry_no; anywhere we need a stable sortable/joinable key, use id.
function displayCode(e) {
  return e.entry_no || String(e.id);
}
...
export function CashFlowPage({ onNavigate }) {
  ...
  const [summary, setSummary] = React.useState({ total_in: 0, total_out: 0, net: 0 });
  ...
  // ক্যাশ ফ্লো এন্ট্রি লিস্ট লোড করা
  const loadEntries = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetchCashFlowEntries();
      setEntries(res.entries || []);
      setSummary(res.summary || { total_in: 0, total_out: 0, net: 0 });
    } catch (err) {
      console.error("Error loading cash flow entries:", err);
      setError("ক্যাশ ফ্লো এন্ট্রি লোড করতে সমস্যা হয়েছে।");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Sorted chronologically (by numeric id — entry_no is a display code,
  // not guaranteed sortable) to compute a running balance.
  const sortedAsc = [...entries].sort((a, b) => a.id - b.id);
  ...
  // Prefer the API's own totals when available; fall back to a client-side
  // sum so the cards still work if `summary` is ever missing.
  const totalIn = summary.total_in ?? entries.filter((e) => e.type === "in").reduce((s, e) => s + e.amount, 0);
  const totalOut = summary.total_out ?? entries.filter((e) => e.type === "out").reduce((s, e) => s + e.amount, 0);
  const net = summary.net ?? totalIn - totalOut;
  ...
  const handleSave = async (form) => {
    try {
      ...
      alert("এন্ট্রি সেভ করা সম্ভব হয়নি।");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      alert("এন্ট্রি ডিলিট করতে সমস্যা হয়েছে।");
    }
  };
    ...
    <div className="p-6" style={{ backgroundColor: COLORS.paper, fontFamily: FONTS.BODY, minHeight: "100%" }}>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: COLORS.panel, borderColor: COLORS.line }}>
        {loading ? (
          ...
          <div className="text-center py-8" style={{ color: red }}>{error}</div>
        ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] border-collapse">
            <thead>
              <tr>
                {["Ref", "Date", "Type", "Source", "Category", "Note", "Amount", "Balance", ""].map((label) => (
              ...
              {filtered.map((e) => {
                ...
                return (
                  <tr key={e.id} className="border-b hover:bg-black/[0.02] transition-colors" style={{ borderColor: COLORS.line }}>
                    <td className="px-5 py-3.5 whitespace-nowrap font-semibold" style={{ color: COLORS.muted, fontFamily: FONTS.MONO, fontSize: 12.5 }}>
                      {displayCode(e)}
                    </td>
