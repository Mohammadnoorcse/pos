...
export function CreateTransferPage({ mode = "B2B_B2G", onConfirm }) {
  ...
  const senderOptions = isG2B ? branches.filter((b) => b.type === "godown") : branches;
  const receiverOptionsFor = (senderId) =>
    isG2B
      ? branches.filter((b) => b.type === "shop" && b.id !== senderId)
      : branches.filter((b) => b.id !== senderId);

  const [step, setStep] = React.useState(1);
  const [sender, setSender] = React.useState(""); // branch id
  ...
  const senderName = branches.find((b) => b.id === sender)?.name || "";
  const receiverName = branches.find((b) => b.id === receiver)?.name || "";
