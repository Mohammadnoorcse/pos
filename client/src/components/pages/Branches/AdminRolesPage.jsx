...
export function AdminRolesPage({ onOpenPermissions }) {
  ...
  const loadRoles = async () => {
    try {
      ...
      setError("এডমিন রোল লিস্ট লোড করতে সমস্যা হয়েছে।");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRole = async (name) => {
    try {
      ...
      alert("রোল তৈরি করা সম্ভব হয়নি। ইউনিক নাম ব্যবহার করুন।");
    }
  };
  ...
  const handleUpdateRole = async (name) => {
    ...
    try {
      await updateAdminRole(editingRole.id, { name });
      setEditingRole(null);
      loadRoles();
    } catch (err) {
