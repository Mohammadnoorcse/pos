...
const INITIAL_BANKS = [
  {
    ...
    accountName: "Moon Traders & Co.",
    accountNumber: "20503450201458963",
    routingNumber: "125270458",
    type: "Current",
  ...
  },
  {
    ...
    accountName: "Moon Traders & Co.",
    accountNumber: "1451200091423",
    routingNumber: "090261455",
    type: "Current",
  ...
  },
  {
    ...
    accountName: "S.M. Sohag Ahmed",
    accountNumber: "7710192834455",
    routingNumber: "060260760",
    type: "Savings",
  ...
  },
  {
    ...
    branch: "Bogura Sadar Branch",
    accountName: "Moon Traders & Co.",
    accountNumber: "3355012209981",
    routingNumber: "200658215",
    type: "OD / CC",
    openingBalance: 0,
    balance: -84200,
    status: "frozen",
...
function bankOf(id) {
  return INITIAL_BANKS.find((b) => b.id === id);
}
...
function BankDrawer({ bank, onClose, onSave, onDelete, mode }) {
  ...
  const tx = bank ? RECENT_TX.filter((t) => t.bank === bank.id) : [];
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: COLORS.muted }}>
                {isNew ? "New account details" : `${bank.id} · ${bank.branch}`}
              </p>
            </div>
          </div>
...
export function BankDetailsPage({ onNavigate }) {
  const [banks, setBanks] = React.useState(INITIAL_BANKS);
  const [query, setQuery] = React.useState("");
  const [typeFilter, setTypeFilter] = React.useState("ALL");
  const [statusFilter, setStatusFilter] = React.useState("ALL");
  const [drawer, setDrawer] = React.useState({ mode: null, bank: null });
  ...
  const handleSave = (form, isNew) => {
    if (isNew) {
      const id = "BNK-" + String(banks.length + 1).padStart(3, "0");
      const newBank = {
        ...form,
        id,
      ...
      notify(`${newBank.bankName} account added`);
    } else {
      setBanks((prev) =>
        prev.map((b) =>
          b.id === form.id
            ? { ...b, ...form, openingBalance: Number(form.openingBalance) || 0, balance: Number(form.balance) || 0 }
            : b
        )
      );
