...
const INITIAL_CONTRAS = [
  {
    ...
    to: "cash",
    amount: 40000,
    note: "Cash withdrawal for weekly wages",
    status: "completed",
    ref: "CHQ-77120",
  },
  {
    ...
    to: "BNK-002",
    amount: 85000,
    note: "Counter cash deposited to bank",
    status: "completed",
    ref: "DEP-33410",
  },
  {
    ...
    to: "BNK-003",
    amount: 150000,
    note: "Fund transfer for Gulshan branch payroll",
    status: "pending",
    ref: "NEFT-90223",
  },
  {
    ...
    to: "BNK-004",
    amount: 60000,
    note: "Covering Sonali Bank OD shortfall",
    status: "completed",
    ref: "RTGS-44871",
  },
  {
...
export function ContraTransferPage({ onNavigate }) {
  const [contras, setContras] = React.useState(INITIAL_CONTRAS);
  ...
  const handleSave = (form) => {
    const nextNum = Math.max(...contras.map((c) => parseInt(c.id.split("-")[1], 10))) + 1;
    const newContra = { id: "CNT-" + String(nextNum).padStart(4, "0"), status: "pending", ...form };
    setContras((prev) => [newContra, ...prev]);
    setDrawerOpen(false);
    notify(`${taka(form.amount)} transfer posted from ${accountName(form.from)} to ${accountName(form.to)}`);
  };

  const handleComplete = (id) => {
    setContras((prev) => prev.map((c) => (c.id === id ? { ...c, status: "completed" } : c)));
    notify(`${id} marked completed`);
  };
                  ...
                  <tr key={c.id} className="border-b hover:bg-black/[0.02] transition-colors" style={{ borderColor: COLORS.line }}>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2.5">
                        ...
                        <button onClick={() => handleDelete(c.id)} style={{ color: COLORS.muted }}>
                          <Trash2 size={12} />
                        </button>
                      </div>
                    </td>
