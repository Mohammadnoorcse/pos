...
/* ---------------------------------------------------------------------
   ...
   Flow / Contra Transfer / Take Customer Due pages (warm paper
   surface, vermillion primary accent, magenta secondary). This page
   inherits that house style since it lives in the same product
   (Acc & Transaction Wing → Supplier Payment).
--------------------------------------------------------------------- */
const COLORS = {
  paper: "#FAF8F4",
  panel: "#FFFFFF",
...
const INITIAL_SUPPLIERS = [
  ...
  {
    id: "SUP-202",
    name: "Padma Edible Oil Ltd.",
    phone: "01822-441122",
    area: "Chattogram",
    totalDue: 258600,
    agingDays: 14,
    lastBill: "BILL-7745",
    lastPaymentDate: "14-07-2026",
  },
  {
    id: "SUP-203",
    ...
    agingDays: 5,
    lastBill: "BILL-7760",
    lastPaymentDate: "18-07-2026",
  },
  {
    id: "SUP-204",
    name: "Sonar Bangla Packaging",
    phone: "01644-227799",
    area: "Gazipur",
    totalDue: 39200,
    agingDays: 46,
  ...
  {
    id: "SUP-206",
    name: "United Grain Traders",
    phone: "01766-330055",
    area: "Dinajpur",
    totalDue: 96700,
    agingDays: 33,
    lastBill: "BILL-7688",
    lastPaymentDate: "22-06-2026",
  },
];

const INITIAL_PAYMENTS = [
  { id: "PAY-6610", date: "21-07-2026", supplier: "SUP-203", account: "cash", amount: 30000, note: "Partial payment against BILL-7760" },
  { id: "PAY-6608", date: "20-07-2026", supplier: "SUP-202", account: "BNK-002", amount: 100000, note: "Cheque issued" },
  { id: "PAY-6605", date: "18-07-2026", supplier: "SUP-205", account: "BNK-001", amount: 60000, note: "Bank transfer" },
];
...
/* ---------------------------------------------------------------------
   Signature element: a "payment voucher" slip.
   Bangladeshi wholesale houses issue a পেমেন্ট ভাউচার the moment cash
   or cheque goes out to a supplier — showing supplier, amount paid,
   remaining payable after this payment, and paid-from account. This
   drawer mirrors that voucher, with the remaining-payable figure
   recalculating live as the payer types — the mirror image of the
   customer collection receipt, but drawn from vermillion (money
   leaving) rather than green (money arriving).
--------------------------------------------------------------------- */
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0" style={{ borderColor: COLORS.line }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: COLORS.muted }}>{supplier.id} · {supplier.area}</p>
            </div>
          </div>
            ...
            <Field label="Note">
              <div className="flex items-start gap-2 rounded-lg border px-3 py-2" style={{ borderColor: COLORS.line }}>
                ...
                <textarea
                  value={form.note}
                  onChange={set("note")}
                  placeholder="e.g. Partial payment against BILL-7760"
                  rows={2}
...
export function SupplierPaymentAcc({ onNavigate }) {
  ...
  const filtered = suppliers
    ...
    .filter((s) => {
      const matchesQuery = [s.name, s.phone, s.area, s.id].join(" ").toLowerCase().includes(query.toLowerCase());
      const matchesAging =
        agingFilter === "ALL" ||
  ...
  const paidToday = payments.filter((p) => p.date === TODAY).reduce((s, p) => s + p.amount, 0);

  const handleSave = (supplierId, form) => {
    const sup = supplierOf(suppliers, supplierId);
    setSuppliers((prev) =>
      prev.map((s) => (s.id === supplierId ? { ...s, totalDue: Math.max(s.totalDue - form.amount, 0), agingDays: 0 } : s))
    );
    const nextNum = Math.max(...payments.map((p) => parseInt(p.id.split("-")[1], 10)), 6600) + 1;
    setPayments((prev) => [{ id: "PAY-" + nextNum, supplier: supplierId, ...form }, ...prev]);
    setSelected(null);
    notify(`${taka(form.amount)} paid to ${sup.name}`);
  };
      ...
      <div className="flex flex-wrap items-center gap-2.5 mb-4">
        <div className="flex items-center gap-2 rounded-lg px-3 py-2 border flex-1 min-w-[220px]" style={{ borderColor: COLORS.line, backgroundColor: COLORS.panel }}>
          ...
          <input
            ...
            placeholder="Search supplier name / phone / area"
            className="bg-transparent outline-none text-[13px] w-full"
            style={{ color: COLORS.ink }}
          />
        </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: COLORS.panel, borderColor: COLORS.line }}>
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] border-collapse">
            <thead>
              <tr>
                {["Supplier", "Contact", "Last bill", "Aging", "Payable amount", ""].map((label) => (
                    ...
                    <td className="px-5 py-3.5" style={{ color: COLORS.ink }}>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[11px]" style={{ color: COLORS.muted }}>{s.area}</div>
                        </div>
                      </div>
                    </td>
